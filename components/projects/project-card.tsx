  ...
  return (
    <Link href={`/projects/${project.id}`}>
      <div className="bg-white/90 backdrop-blur-sm rounded-xl shadow-md hover:shadow-2xl hover:-translate-y-1 transition-all duration-300 p-6 cursor-pointer border border-slate-200 hover:border-appcollab-teal group h-full flex flex-col relative">
        ...
        <div className="flex flex-wrap items-center justify-between text-xs text-gray-500 pt-4 border-t border-gray-100 mt-auto gap-y-2 gap-x-2">
          <span className="font-medium whitespace-nowrap">{new Date(project.created_at).toLocaleDateString()}</span>

          <div className="flex flex-wrap items-center gap-2 flex-1 justify-center px-2 min-w-fit">
